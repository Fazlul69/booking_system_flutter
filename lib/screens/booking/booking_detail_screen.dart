...
class _BookingDetailScreenState extends State<BookingDetailScreen> {
  ...
  Widget _buildReasonWidget({required BookingDetailResponse snap}) {
    if (((snap.booking_detail!.status == BookingStatusKeys.cancelled || snap.booking_detail!.status == BookingStatusKeys.rejected || snap.booking_detail!.status == BookingStatusKeys.failed) &&
        ((snap.booking_detail!.reason != null && snap.booking_detail!.reason!.isNotEmpty))))
      return Container(
        ...
        child: Column(
          ...
          children: [
            Text(getReasonText(snap.booking_detail!.status.validate()), style: primaryTextStyle(color: redColor, size: 18)),
            6.height,
            Text('${snap.booking_detail!.reason.validate()}', style: secondaryTextStyle()),
          ],
        ),
      );
    ...
    return Column(
      ...
      children: [
        ...
        bookingDetail.description.validate().isNotEmpty
            ...
            : Text('No Description Available', style: secondaryTextStyle()).center(),
        16.height,
      ],
    );
  ...
  Widget _action({required BookingDetailResponse status}) {
    ...
    } else if (status.booking_detail!.status == BookingStatusKeys.complete && (status.booking_detail!.paymentStatus == null)) {
      return AppButton(
  ...
  void resumeClick({required BookingDetailResponse status}) async {
    ...
    await updateBooking(request).then((res) async {
      ...
      commonStartTimer(isHourlyService: status.booking_detail!.isHourlyService, status: BookingStatusKeys.inProgress, timeInSec: status.booking_detail!.durationDiff.validate().toInt());
      setState(() {});
    }).catchError((e) {
  ...
  void doneClick({required BookingDetailResponse status}) async {
    ...
    await updateBooking(request).then((res) async {
      toast(res.message!);
      commonStartTimer(isHourlyService: status.booking_detail!.isHourlyService, status: BookingStatusKeys.complete, timeInSec: status.booking_detail!.durationDiff.validate().toInt());
    }).catchError((e) {
      toast(e.toString(), print: true);
    });
                      ...
                      Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (snap.data!.serviceProof.validate().isNotEmpty) _serviceProofListWidget(list: snap.data!.serviceProof.validate()),
                            ...
                            28.height,
                            PriceCommonWidget(bookingDetail: snap.data!.booking_detail!, serviceDetail: snap.data!.service!, taxes: snap.data!.booking_detail!.taxes.validate(), couponData: snap.data!.couponData),
                            16.height,
